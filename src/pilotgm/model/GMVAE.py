...
class GMVAE:
  ...
  def __init__(self, args):
    ...
    self.labels_column= args.labels_column
  ...
  def unlabeled_loss(self, data, out_net):
    ...
    # reconstruction loss
    loss_rec = self.losses.reconstruction_loss(data, data_recon, self.rec_type)  #−logp(x∣z)

    # gaussian loss
    loss_gauss = self.losses.gaussian_loss(z, mu, var, y_mu, y_var)  #DKL(q(z∣ x,y)∥ p(z∣y))

    # categorical loss
    loss_cat = -self.losses.entropy(logits, prob_cat) - np.log(1/self.num_classes) #DKL(q(y∣x)∥ p(y))
  ...
  def infer(self, data_loader):
    """Infer latent features, reconstructed data, cluster probabilities, and cluster assignments.

    Args:
        data_loader: (DataLoader) loader containing the data.

    Returns:
        z_latent: (array) Latent variable \( z \).
        x_recon: (array) Reconstructed \( x \).
        cluster_probs: (array) Soft probabilities for each cluster \( q(y|x) \).
        clusters: (array) Hard cluster assignments (argmax of probabilities).
    """
    self.network.eval()
    z_latent, x_recon, cluster_probs, clusters = [], [], [], []
    ...
    with torch.no_grad():
        for data, _ in data_loader:
            ...
            # Forward pass
            out = self.network(data, self.gumbel_temp, self.hard_gumbel)
            
            # Latent variables and outputs
            z = out['mean'].cpu().numpy()  # Latent \( z \)
            x_reconstructed = out['x_rec'].cpu().numpy()  # Reconstructed \( x \)
            probs = out['prob_cat'].cpu().numpy()  # Soft probabilities for clusters
            cluster_assignments = np.argmax(probs, axis=1)  # Hard cluster assignments
            
            # Append results
            z_latent.append(z)
            x_recon.append(x_reconstructed)
            cluster_probs.append(probs)
            clusters.append(cluster_assignments)
    
    # Concatenate results for all batches
    z_latent = np.vstack(z_latent)
    x_recon = np.vstack(x_recon)
    cluster_probs = np.vstack(cluster_probs)
    clusters = np.concatenate(clusters)
    
    return z_latent, x_recon, cluster_probs, clusters
